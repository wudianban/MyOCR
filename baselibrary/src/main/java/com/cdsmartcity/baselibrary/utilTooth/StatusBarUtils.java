...
public class StatusBarUtils {
    ...
    public StatusBarUtils setDark(boolean dark) {
        mDark = dark;
        return this;
    }
...
/*        if (mDark) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP_MR1) {
                setMeizuStatusBarDarkIcon(mActivity, mDark);
                setMiuiStatusBarDarkMode(mActivity, mDark);
            }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (mDark) {
                    mActivity.getWindow().getDecorView().setSystemUiVisibility(SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
                }
            }
        }*/
    }
